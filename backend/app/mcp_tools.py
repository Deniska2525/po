...
Это отдельная от app/ai_search.py вещь:
  - app/ai_search.py — наш backend сам ходит во внешний LLM (по умолчанию —
    Gemma 4 через OpenRouter, см. AI_BASE_URL/AI_MODEL), чтобы обслужить
    ИИ-поиск на нашей же главной странице.
...
mcp = FastMCP(
    ...
    # ВАЖНО: streamable_http_app() сам по себе монтирует обработчик на "/mcp"
    # внутри своего Starlette-приложения. Если потом ещё раз смонтировать это
    # приложение на "/mcp" в main.py, итоговый путь станет "/mcp/mcp", а не
    # "/mcp". Явно задаём internal path="/", чтобы после монтирования в
    # main.py путь получился ровно "/mcp".
